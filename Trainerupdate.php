...
$RegID = $_POST["RegID"];
$sql = "SELECT * FROM `trainer` WHERE `trainer_id` = $RegID";
...
<body style="background-image: url(gymbackground.jpg); background-size:100%;  background-repeat: no-repeat;">
    <div class="square">
        <div class="content">
            <a href="mainmenu.html" class="backbutton">&times;</a>
            <h1>Trainer Updation Form</h1>
            <form id="myForm" action="TrainerUpdateTable.php" method="POST">
                <div class="user-container">
                    <div class="user-box" style="flex-direction: column;">
                        <br>
                        Registration number:<input type="hidden" value="<?php echo "$RegID"; ?>"
                            name="RegID"><?php echo $RegID; ?><br><br>
                        <label>First Name:</label>
                        <input type="text" id="fname" name="fname" size="20" value="<?php echo "$firstname"; ?>"
                            onkeydown="move(event,'lname')" required /><br><br>
                        <label>Last Name:</label>
                        <input type="text" id="lname" name="lname" size="20" value="<?php echo "$lastname"; ?>"
                            onkeydown="move(event,'phone_number')" required /><br><br>
                        <label>Ph No:</label>
                        <input type="text" id="c_code" size="1" value="+91" />
                        <input type="number" id="phone_number1" name="phone_number1" size="11"
                            value="<?php echo "$phone_number"; ?>" onkeydown="move(event,'email')" required /><br><br>
                        <label>Email ID:</label>
                        <input type="email" id="email" name="email" size="20" value="<?php echo "$email_id"; ?>"
                            onkeydown="move(event,'address')" required />
                        <br><br>
                        <label>Address:</label>
